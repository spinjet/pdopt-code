...
from pdopt.bn_prototype import BN_Exploration
    ...
    def run(self, *args, **kwargs):
        ...
        if any(checks):
            ...
            if "motor" in kwargs.keys():
                ...
                self.arch["eta_e"] = np.prod(list(self.arch["eta_e_comp"].values()))
        ...
        analysis = model(
            en_management, architecture_data=self.arch, mission_file=self.mission
        )
...
if __name__ == "__main__":
    # run experiment with the input set inside the filerightmove
    case_folder = "test_case_linear2"
    P_sat = 0.5
    n_exp_samples = 100
    n_train_samples = 100
    restart = False
