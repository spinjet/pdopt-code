...
def generate_surrogate_training_data(
    parameters_list, model, n_train_points, save_dir=None, debug=False
):
...
def generate_surrogate_test_data(
    n_points, parameters_list, model, save_dir=None, debug=False
):
...
class SurrogateResponse:
    ...
    def __init__(
        self, response_name, parameters_list, model, train_data=None, test_data=None
    ):
        ...
        if self.score < 0.6:
            ...
            raise (
                "Errore: Surroate {} score is negative, r={:.4f}".format(
                    self.name, self.score
                )
            )
