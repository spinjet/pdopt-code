...
# Local imports
#from pdopt.data import DesignSpace, Model
...
def generate_parallel_coordinates(color_label):
    ...
    if len(g_selected_points_parc) > 0:
        ...
        if color_label != 'none':
            ...
            color_scale = [[color_values[i], PARCOORD_COLOR_LIST[i]] for i in range(len(PARCOORD_COLOR_LIST))]
            ...
            parcoord = go.Parcoords(
                    ...
                    dimensions = list([dict(range = [df[col].min(), df[col].max()], 
                                            label  = '<b>*{}*</b>'.format(col) if col == color_label else col, 
            ...
            color_scale = [[0, 'rgb(211,211,211)'], [0.5, 'rgb(139,0,0)'], [1, 'rgb(139,0,0)']]
            #color_scale = [[0, 'rgb(211,211,211)']] + color_scale
...
def generate_scatter(x_label, y_label, color_label, scatter_id):
    ...
    hovertemplate="<br>".join(
        ['{}:%{{customdata[{}]:.3f}}'.format(g_dataframe.columns[k], k) for k in range(len(g_dataframe.columns))]
        ))
...
def update_parallel_coordinates(c_label, dummy):
    ...
    if len(g_par_coord_selected_ranges) > 0:
        ...
        for axis, values in g_par_coord_selected_ranges.items():
            g_parallel_coord_figure.data[0].dimensions[int(axis)].constraintrange = values
...
def get_selected_data_parc(scatter_data1, scatter_data2, scatter_data3, 
    ...
    scatter1_points = [p['pointIndex'] for p in scatter_data1['points']] if scatter_data1 else []
    scatter2_points = [p['pointIndex'] for p in scatter_data2['points']] if scatter_data2 else []
    scatter3_points = [p['pointIndex'] for p in scatter_data3['points']] if scatter_data3 else []
    scatter4_points = [p['pointIndex'] for p in scatter_data4['points']] if scatter_data4 else []
    scatter5_points = [p['pointIndex'] for p in scatter_data5['points']] if scatter_data5 else []
    ...
    if len(rows_from_scatters) > 1:
        g_selected_points_parc = list(reduce(set.intersection, [set(x) for x in rows_from_scatters]))
...
def get_selected_data_scatters(restyleData, scatter_data1, scatter_data2, scatter_data3,
    ...
    if len(rows_from_scatters) > 1:
        g_selected_points_sca = list(reduce(set.intersection, [set(x) for x in rows_from_scatters]))
    elif len(rows_from_scatters) == 1:
...
def main_inline(dataframe, debug=False):
    ...
    g_customdata = np.empty(shape=(len(g_dataframe),len(g_dataframe.columns),1), dtype='object')
    ...
    def load_data(file_contents, filename):
        ...
        if file_contents is None or file_contents is []:
            ...
            try:
                ...
                g_parallel_coord_figure = generate_parallel_coordinates(color_label=default_value)
