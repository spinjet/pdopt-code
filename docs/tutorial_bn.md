...
Starting from version 0.6, PDOPT uses Bayesian Networks to perform the exploration phase. The advantage of this approach over the previous one is greater flexibility in discarding the design sets that are not of interest based on a custom query (in place of a fixed satisfy the requirements). Furthermore, it opens up the possibility to handle uncertainty in the constraints themselves.
