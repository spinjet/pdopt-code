...
def test_is_pareto_efficient():
    ...
    expected_pareto = np.array([[0.09417735, 0.97562235],
...
def test_generate_run_report():
    ...
    for i_set in my_Optimisation.valid_sets_idx:
        if my_DesignSpace.sets[i_set].optimisation_results is not None:
            optimisation_time.update(
                {i_set: my_DesignSpace.sets[i_set].opt_dt})
    ...
    out = tools.generate_run_report('test_report.txt', my_DesignSpace, 
                              my_Optimisation, my_Exploration)
